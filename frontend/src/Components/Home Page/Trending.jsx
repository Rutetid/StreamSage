...
export default function Trending({
				...
				const response = await axios.get(
					"https://api.themoviedb.org/3/movie/now_playing?language=en-US&page=1",
					{
						headers: {
							...
							Authorization:
								"Bearer eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiJmNWM3ZmRlYjU1ZWM5OTFjZmRjNDg3MzljOGY0MzQwYSIsIm5iZiI6MTczMTI0NDk5Ny4yMTUzNzEsInN1YiI6IjY2ODdiNTVkZDk1MzFlMjQ1YjA4MTM0MCIsInNjb3BlcyI6WyJhcGlfcmVhZCJdLCJ2ZXJzaW9uIjoxfQ.WeCIYhgvJ8nRnR0F-jlDEsM4oViGpc63L5IFo9ci1Wo",
						},
					},
				);
	...
	const prev = () =>
		setCurr((curr) => (curr === 0 ? movies.length - 1 : curr - 1));
	const next = () =>
	...
	return (
		<div className="overflow-hidden relative w-full bg-[040a10] py-2 sm:py-4 md:py-6">
			<div
			...
			>
				{movies.length > 0 ? (
					movies.map((movie) => (
						...
						>
							<div className="relative group overflow-hidden rounded-lg shadow-lg w-[95%] sm:w-[90%] md:w-[85%] lg:w-[60%] mx-auto">
								...
								<div className="absolute bottom-4 left-4 right-4 flex justify-between items-end group-hover:opacity-0 group-hover:translate-y-[-100px] group-hover:translate-x-[16px] transition-all duration-800">
									<h3 className="text-white text-xl md:text-2xl font-poppins font-semibold line-clamp-2">
								...
								<div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-800">
									<div className="absolute bottom-4 left-4 right-4 flex flex-col items-start p-4 transform translate-y-[40px] group-hover:translate-y-0 transition-transform duration-800">
										...
										<p className="text-white text-sm md:text-base mb-4 line-clamp-3">
											{movie.overview}
										</p>
										<button
											type="button"
        ...
        type="button"
        onClick={prev}
        className="absolute left-1 sm:left-2 md:left-4 top-1/2 -translate-y-1/2 bg-gray-100/20 text-gray-800 p-1 sm:p-2 rounded-full shadow hover:bg-white transition-colors duration-200"
      ...
      <button
        type="button"
        onClick={next}
        className="absolute right-1 sm:right-2 md:right-4 top-1/2 -translate-y-1/2 bg-white/80 text-gray-800 p-1 sm:p-2 rounded-full shadow hover:bg-white transition-colors duration-200"
